...
public class DynamicArray<E> implements Iterable<E>  {
    ...
    private static final int DEFAULT_CAPACITY = 16;//默认的动态数组的容量
    private int capacity;//实际的动态数组的容量
    private int size;//真实数据的长度
    ...
    public void add(final E element) {
        if (this.size == this.elements.length) {//数组已满，则扩容一个两倍的数组，将原来的数据复制过去
    ...
    public E remove(final int index) {
        ...
        fastRemove(this.elements, index);//这里已经删除了原来元素

        if (this.capacity > DEFAULT_CAPACITY && size * 4 <= this.capacity)
            //若删除后的数组大小的4倍<现在的容量&&现在的容量>默认容量 则数组的容量缩小为现有容量的一半
    ...
    private void fastRemove(final Object[] elements, final int index) {
        ...
        if (newSize > index) {
            //原数组，需要复制的其实索引，目标数组，目标数组开始的索引位置，需要复制的长度
